...
class GoogleTTSClient:
    ...
    async def generate_speech_pcm(self, text_chunk: str, voice_name: str = None, system_prompt: str = None) -> bytes:
        ...
        selected_voice = voice_name or self.config.voices.get("default_voice", "Puck")

        # Build request config with response_modalities=["AUDIO"]
        ...
        for attempt in range(1, retry_attempts + 1):
            try:
                ...
                err_msg = str(e).lower()
                is_rate_limit = "429" in err_msg or "resource_exhausted" in err_msg or "quota" in err_msg or "503" in err_msg
                if is_rate_limit and attempt < retry_attempts:
                    ...
                    logger.warning(
                        f"[RATE LIMIT 429/503] API quota/rate limit hit. Intentional delay: backing off for {sleep_time:.1f}s before retry (Attempt {attempt}/{retry_attempts})..."
                    )
